...
コマンドを実行しても Quick Pick が表示されないときは、**Esc キーを押して一度キャンセルしてから、エディター上でファイルを開き直し、ファイル上で右クリック**してコマンドを再実行してください。

エクスプローラーのサイドバー（ファイルツリー）ではなく、エディター中央のファイル本体を右クリックすることで、カーソル位置のメソッド情報を正しく読み取れます。

`@Template` アノテーションや `render()` の記述がないコントローラーをエクスプローラーから右クリックした場合でも、v0.0.9 からは全候補を一覧表示する Quick Pick が表示されるようになりました。それでも表示されない場合は、まず Esc キーで VS Code のポップアップを閉じてから再度お試しください。
...
- **修正: `@Template` や `render()` の記述がないコントローラーをエクスプローラーから右クリックしたとき、Quick Pick が表示されない問題を修正しました。** エクスプローラーから右クリックした場合にアクティブエディターが別ファイルだと、カーソル行が意味のない値になりメソッド解析が失敗して即 return していました。エクスプローラーからの実行時はカーソル行を無視し、テンプレート参照がなくても全候補を一覧表示するフォールバックピッカーを表示するよう改善しました。
...
- カーソルをアクションメソッドの任意の位置（関数シグネチャの行を含む）に置いても、関数定義の直前に書かれた `@Template` アノテーションが正しく認識されるようになりました。
